...
class PreTrainedEmbeddings:
    ...
    def __init__(self):
        self.dim = config.Text2PermissionClassifier.embedding_dim
    ...
    def load(self):
        if self.is_cached():
            print("loading cached word embeddings file: ",
                  config.Text2PermissionClassifier.cached_embedding_file_values,
                  " -- ",
                  config.Text2PermissionClassifier.downloaded_embedding_file)
            self.load_cached()
        elif os.path.exists(config.Text2PermissionClassifier.downloaded_embedding_file):
            print("loading raw word embeddings file: ", config.Text2PermissionClassifier.downloaded_embedding_file)
            self.create_cached()
        else:
    ...
    def is_cached(self):
        cached_file = config.Text2PermissionClassifier.cached_embedding_file_values
        check_file = config.Text2PermissionClassifier.cached_embedding_file_check
        downloaded_file = config.Text2PermissionClassifier.downloaded_embedding_file
    ...
    def create_cached(self):
        ...
        with open(config.Text2PermissionClassifier.downloaded_embedding_file) as f:
        ...
        embedding_matrix = np.memmap(config.Text2PermissionClassifier.cached_embedding_file_values,
                                     dtype='float32',
                                     mode='w+',
        ...
        pickle.dump(word2index, open(config.Text2PermissionClassifier.cached_embedding_file_indices, "wb"))

        # for cache validity check
        open(config.Text2PermissionClassifier.cached_embedding_file_check, "w")\
            .write(str(os.stat(config.Text2PermissionClassifier.downloaded_embedding_file).st_size))

        print("saved raw embedding")

    def load_cached(self):
        self.word2index_dict = pickle.load(open(config.Text2PermissionClassifier.cached_embedding_file_indices, "rb"))
        shape = (len(self.word2index_dict), self.dim)
        self.embedding_matrix = np.memmap(config.Text2PermissionClassifier.cached_embedding_file_values,
